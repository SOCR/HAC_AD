...
```{r}
# today_date = format(Sys.time(), "%m%d%Y")
# filename = "compare_disambiguation.csv" # note, this needs to be changed
# csv_save_path = paste("data/", today_date, filename , sep="")
# 
# write.csv(text.df, file=csv_save_path)
```
...
```{r}
# today_date = format(Sys.time(), "%m%d%Y")
# filename = "compare_disambiguation_and_preprocessing.csv" # note, this needs to be changed
# csv_save_path = paste("data/", today_date, filename , sep="")
# 
# write.csv(text.df, file=csv_save_path)
```
...
dtm.tfidf$dimnames$Docs <- as.character(text.df$STUDY_NOTE_ID)
```
...
```{r}
dtm.tfidf_long <- tidy(dtm.tfidf, row_names=dtm.tfidf$dimnames$Docs, col_names=dtm.tfidf$dimnames$Terms)
dtm.tfidf_df <- dcast(dtm.tfidf_long, document ~ term , value.var = 'count' )
dtm.tfidf_df <- dtm.tfidf_df %>% replace(is.na(.), 0) # this is slow
colnames(dtm.tfidf_df)[colnames(dtm.tfidf_df)=="document"] <- "STUDY_NOTE_ID"
```


```{r}
dtm.tfidf_df <- dtm.tfidf_df[, !duplicated(colnames(dtm.tfidf_df))] # remove duplicate columns

sum(duplicated(colnames(dtm.tfidf_df)))
# dtm.tfidf_df[duplicated(lapply(dtm.tfidf_df, summary))]
```
...
# maybe try cbind? # new_features_df <- cbind(text.df, dtm.tfidf_df)
########################## TODO ########################## 

new_features_df <- merge(x = text.df,
                         y = dtm.tfidf_df,
                         by.y=c('STUDY_NOTE_ID'),
...
```{r}
ncol(new_features_df) # expect 104
```
