...
```{r}
enc_df <- read_multi_character_delim(encounters_path, "<break>", header = TRUE)
head(enc_df)
```

It looks like 'Labs' is already a `.csv` file....
...
```{r}
# note_df <- read_multi_character_delim(notes_path, "<break>", header = TRUE)
# head(note_df)
```
