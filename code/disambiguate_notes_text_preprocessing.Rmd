...
```{r}
# relative_sample_path <- "data/DS_PreprocessedAnonymized_Text.xlsx" # osx
# relative_sample_path <- "../../local_data/DS_PreprocessedAnonymized_Text.xlsx" # win
# raw_data_df <-  read_xlsx_from_path(filepath = relative_sample_path)
```
...
```{bash}
head -n 2 "data/full_set/Notes.dsv"
```

```{bash}
tail -n 10 "data/full_set/Notes.dsv"
```
...
```{bash}
sed -n "178791,178794p" "data/full_set/Notes.dsv"
```
...
text_df <- subset_dataframe(sampled_df, cols) # note, working with sampled DF
...
raw_frequencies <- get_token_frequecy_from_column(text_df, 'TEXT', sortByHighFQ=TRUE)
...
```{r}
nrow(text_df) # expect 104 on testing data
```
...
```{r}
perform_text_preprocessing_operations <- function(columnVector, stripName=TRUE, rmStopWords = FALSE, stemResults=FALSE) {
...
doc_corpus <- perform_text_preprocessing_operations(text_df$card_disambiguated_note)
...
dtm.tfidf <- build_tfidf_matrix(doc_corpus, text_df$STUDY_NOTE_ID)
```
