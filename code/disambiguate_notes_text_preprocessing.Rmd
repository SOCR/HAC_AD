...
```{r}
build_tfidf_matrix <- function(corpus, dictionary, ID_vector) {
  ...
  if (missing(dictionary)) {
   ...
   print(paste0('NEEED! Dictionary procedure. ', dictionary)) 
  }
}

dtm.tfidf <- build_tfidf_matrix(corpus = doc_corpus, ID_vector = text_df$STUDY_NOTE_ID)
...
```{r}
cols = c('STUDY_NOTE_ID','processed_text')
tmp_df <- subset_dataframe(text_df, cols) # note, working with sampled df

notes_ngrams <- tidytext::unnest_tokens(tmp_df, ngram, 'processed_text', token = "ngrams", n = 3)
...
```{r}
tm::findAssocs(dtm.tfidf, "memori", corlimit = 0.1)
```
